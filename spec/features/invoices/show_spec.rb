...
RSpec.describe "invoices show" do
  ...
  describe "link to applied discounts" do
    it "should display link to any applied bulk_discounts next to each invoice item" do
      merchant1 = Merchant.create!(name: "Hair Care")
      ...
      visit merchant_invoice_path(merchant1, invoice_1)
      # save_and_open_page
      invoice_1.bulk_discount_applied

      within("#the-status-#{ii_1.id}") do
        # expect(page).to have_content("Bulk Discount #{bd_1.id}")
      end
    end
  end
